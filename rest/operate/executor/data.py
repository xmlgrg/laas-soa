...
def insert():
    ...
    :return:
    """
    request_data = form.check(['business_id', "data_data_id"])  # 校验是否传入 业务id, 数据模型数据id
    business_id = request_data["business_struct_id"]
    data_data_id = request_data["data_data_id"]
    executor_data_id = time.time_ns()  # 执行器数据id, 后续包含: 执行日志、执行状态 等等
    threading.Thread(target=business_hardcode_pool[business_id], args=(executor_data_id, data_data_id,)).start()
    return json.dumps({
